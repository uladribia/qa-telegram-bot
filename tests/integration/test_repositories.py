...
async def test_message_add_is_idempotent_by_external_id() -> None:
    """Re-processing the same external message does not duplicate state."""
    repository = InMemoryMessageRepository()
    assert await repository.add(_message("m1", "ext-1")) is True
    assert await repository.add(_message("m2", "ext-1")) is False
    assert await repository.get("m2") is None
    stored = await repository.get_by_external_id("s1", "ext-1")
    assert stored is not None
    assert stored.id == "m1"


async def test_messages_without_external_id_are_always_added() -> None:
    """Messages with no external id have no idempotency key to collide on."""
    repository = InMemoryMessageRepository()
    assert await repository.add(_message("m1", None)) is True
    assert await repository.add(_message("m2", None)) is True


async def test_qa_versioning_supersedes_without_deleting_history() -> None:
    ...
    await items.save(replace(item, current_version_id="v1"))
    ...
    await versions.add(second)
    current = await items.get("q1")
    assert current is not None
    await items.save(replace(current, current_version_id="v2"))
...
async def test_bot_answers_can_be_listed_in_a_window() -> None:
    """Answer listing is half-open on the end of the window."""
    repository = InMemoryBotAnswerRepository()
    await repository.add(_bot_answer("a1", minutes=0))
    await repository.add(_bot_answer("a2", minutes=60))
    await repository.add(_bot_answer("a3", minutes=120))
    window = await repository.list_between(NOW, NOW + timedelta(minutes=90))
