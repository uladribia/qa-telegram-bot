...
def test_ingest_creates_source_conversation_and_message() -> None:
    """A first message creates the channel rows it depends on."""
    ingestor = _ingestor()
    result = ingestor.ingest(_message())
    assert result.created is True
    source = ingestor.sources.get(SourceType.TELEGRAM.value)
    assert source is not None
    assert source.source_type is SourceType.TELEGRAM
    conversation = ingestor.conversations.get("-100")
    assert conversation is not None
    stored = ingestor.messages.get("tg:-100:10")
    assert stored is not None
    assert stored.text == "Quan entrenen?"


def test_reprocessing_the_same_event_does_not_duplicate() -> None:
    """Idempotency: the same external id is stored once."""
    ingestor = _ingestor()
    assert ingestor.ingest(_message(message_id="a")).created is True
    second = ingestor.ingest(_message(message_id="b", external_id="-100:10"))
    assert second.created is False
    assert ingestor.messages.get("b") is None
    existing = ingestor.messages.get_by_external_id(
...
def test_attachments_are_persisted_as_metadata() -> None:
    """Attachment metadata is stored; no content field exists."""
    ingestor = _ingestor()
    attachment = AttachmentRef(
        kind="image", external_id="file-1", width=1280, height=720
    )
    ingestor.ingest(_message(attachments=[attachment]))
    stored = ingestor.attachments.list_for_message("tg:-100:10")
    assert len(stored) == 1
...
def test_admin_flag_is_persisted() -> None:
    """The admin flag travels from the contract to the stored message."""
    ingestor = _ingestor()
    ingestor.ingest(_message(is_admin=True))
    stored = ingestor.messages.get("tg:-100:10")
    assert stored is not None
